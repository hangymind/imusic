...
if (localStorage.getItem('theme') === 'light') { //肝帝永不没落！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
        ...
        // progressBar元素已移除，现在使用timeSlider控制进度
                ...
                if (text.includes('<') && text.includes('>')) {
                    try {
                        ...
                        if (svgRegex.test(text)) {
                            ...
                            remainingText = text.replace(svgRegex, '').trim();
                        } 
                            ...
                            const svgElement = tempDiv.firstElementChild.cloneNode(true);
                            watermarkDisplay.appendChild(svgElement);
                            remainingText = text.replace(pathRegex, '').trim();
                        }
                        if (remainingText) {
                            if (watermarkDisplay.children.length === 0 || (remainingText.includes('<') && remainingText.includes('>'))) {
                        
                                tempDiv.innerHTML = remainingText;
                                while (tempDiv.firstChild) {
                                    watermarkDisplay.appendChild(tempDiv.firstChild);
                                }
                            } else {
                                if (watermarkDisplay.children.length > 0) {
                                    watermarkDisplay.appendChild(document.createTextNode(' '));
                                }
                                watermarkDisplay.appendChild(document.createTextNode(remainingText));
                            }
                        }
