...
let debugMode = false;
const appVersion = "2.1.02025120618h33min40s"; 
const welcomeMsg = '@所有人 温迪复刻了';
let debugLogEntries = [];
...
return false;
        ...
        //});
        // 隐藏配置选项功能
        const hideControlsToggle = document.getElementById('hideControlsToggle');
        ...
        let isHideControlsEnabled = savedHideControls === 'true';
        if (isHideControlsEnabled) {
            hideControlsToggle.checked = true;
            hideControlButtons(true);
        }
        
        // 事件监听器
        hideControlsToggle.addEventListener('change', () => {
            if (hideControlsToggle.checked) {
                // 显示确认弹窗
                hideControlsConfirmModal.style.display = 'flex';
                void hideControlsConfirmModal.offsetWidth;
                hideControlsConfirmModal.classList.add('show');
            } else {
                localStorage.setItem('hideControls', 'false');
        ...
        // 确认隐藏
        hideControlsConfirmBtn.addEventListener('click', () => {
            localStorage.setItem('hideControls', 'true');
            isHideControlsEnabled = true;
            hideControlButtons(true);
            closeModal('hideControlsConfirmModal');
        });
        
        // 取消隐藏
        hideControlsCancelBtn.addEventListener('click', () => {
            hideControlsToggle.checked = false;
            closeModal('hideControlsConfirmModal');
        });
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'b' || e.key === 'B') {
                if (isHideControlsEnabled) {
                    ...
                    localStorage.setItem('hideControls', 'false');
                    isHideControlsEnabled = false;
                    if (hideControlsToggle) {
                        hideControlsToggle.checked = false;
                    }
                    hideControlButtons(false);
                    showTopRightToast('已取消隐藏配置选项');
                } else {
                    // 触发显示确认框
                    hideControlsToggle.checked = true;
                    hideControlsConfirmModal.style.display = 'flex';
