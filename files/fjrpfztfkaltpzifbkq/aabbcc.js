                ...
                if (text.includes('<') && text.includes('>')) {
                    try {
                        ...
                        if (svgRegex.test(text)) {
                            ...
                            watermarkDisplay.appendChild(svgElement);
                            if (textAfterSvg) {
                                watermarkDisplay.appendChild(document.createTextNode(' '));
                                watermarkDisplay.appendChild(document.createTextNode(textAfterSvg));
                            }
                        } 
                            ...
                            if (pathIndices.length > 0) {
                                ...
                                if (textBeforePath) {
                                    watermarkDisplay.appendChild(document.createTextNode(textBeforePath));
                                    watermarkDisplay.appendChild(document.createTextNode(' '));
                                }
                                ...
                                if (textAfterPath) {
                                    watermarkDisplay.appendChild(document.createTextNode(' '));
                                    watermarkDisplay.appendChild(document.createTextNode(textAfterPath));
                                }
                            }
                        }
                        if (watermarkDisplay.children.length === 0 && !watermarkDisplay.textContent.trim()) {
...
function initChat() {
    ...
    function addLocalMessage(message, isOwn = false) {
    ...
    if (socket) {
        ...
        socket.on('assigned-username', function(username) {
            currentUserNumber = username;
        });
        ...
        socket.on('chat-message', function(data) {
            ...
            messageElement.classList.add('chat-message');
            if (data.username) {
                if (currentUserNumber && data.username === currentUserNumber) {
                    messageElement.classList.add('own');
        ...
        socket.on('user-joined', function(username) {
            ...
            messageElement.textContent = `${username} 加入了聊天`;
            chatMessages.appendChild(messageElement);
            chatMessages.scrollTop = chatMessages.scrollHeight;
        });
        socket.on('user-left', function(username) {
            ...
            const messageElement = document.createElement('div');
            messageElement.classList.add('chat-message', 'system');
            messageElement.textContent = `${username} 离开了聊天`;
            chatMessages.appendChild(messageElement);
            chatMessages.scrollTop = chatMessages.scrollHeight;
        });
    }
