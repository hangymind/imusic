...
let debugMode = false;
const appVersion = "1.9.025101223h32m35s"; 
let debugLogEntries = [];
...
window.addEventListener('load', () => {
    if (showToastOnLoad) {
        showTopRightToast('肘击netlify');//我在这输入文字
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (woodenFishButton && clickCountElement) {
        const audio = new Audio('kick.wav');
        let clickCount = parseInt(localStorage.getItem('woodenFishClickCount')) || 0;
        clickCountElement.textContent = `敲击次数: ${clickCount}`;
        woodenFishButton.addEventListener('click', function() {
            clickCount++;
            clickCountElement.textContent = `敲击次数: ${clickCount}`;
            localStorage.setItem('woodenFishClickCount', clickCount.toString());
            audio.currentTime = 0; 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (lyricsContainer && scrollPanel) {
        lyricsContainer.addEventListener('wheel', function(event) {
         
            const isAtBottom = Math.abs(lyricsContainer.scrollHeight - lyricsContainer.clientHeight - lyricsContainer.scrollTop) < 1;
            
            if (event.deltaY > 0 && isAtBottom && !scrollPanelVisible) {
            ...
            else if (event.deltaY < 0 && scrollPanelVisible) {
