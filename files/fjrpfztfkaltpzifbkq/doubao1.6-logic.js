...
window.exportFavorites = function() {
    try {
        console.log('导出收藏函数被调用');
        const currentFavorites = JSON.parse(localStorage.getItem('musicFavorites')) || [];
        console.log('当前收藏列表:', currentFavorites);
        
        if (!currentFavorites || currentFavorites.length === 0) {
            alert('收藏列表为空，无法导出！');
            return;
        }
        const exportData = currentFavorites.map(song => ({
            title: song.name || song.title || '未知标题',
            artist: song.artist || '未知艺术家',
            url: song.url || '',
            cover: song.cover || '',
            lyric: song.lyric || ''
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `收藏_${new Date().toLocaleDateString()}.json`;
        document.body.appendChild(link);
        link.click();
        setTimeout(() => {
            document.body.removeChild(link);
        ...
        console.error('导出收藏出错:', error);
        alert('导出收藏时发生错误，请查看控制台获取详细信息。');
    }
};
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (importFavoritesBtn) {
        importFavoritesBtn.addEventListener('click', function(e) {
            e.stopPropagation();
    ...
    if (exportFavoritesBtn) {
        exportFavoritesBtn.addEventListener('click', function(e) {
            e.stopPropagation();
        ...
        reader.onload = (ev) => {
            try {
                ...
                if (currentFavorites.length > 0) {
                    shouldReplace = confirm('是否替换现有收藏列表？\n点击"确定"替换并导入，点击"取消"追加到现有列表。');
                }
                
                let updatedFavorites;
                if (shouldReplace) {
                    // 替换现有收藏列表
                    updatedFavorites = importedSongs;
                } else {
                    // 合并收藏列表，避免重复
                    updatedFavorites = [...currentFavorites];
                    importedSongs.forEach(importedSong => {
                        // 检查是否已存在相同URL的歌曲
                        const isDuplicate = updatedFavorites.some(fav => fav.url === importedSong.url);
                        if (!isDuplicate) {
                            updatedFavorites.push(importedSong);
                        }
                    });
                }
                ...
                // 检查是否有本地路径的歌曲
                const hasLocalPaths = updatedFavorites.some(song => 
                    song.url.startsWith('file://') || 
                    /^[a-zA-Z]:\\|^\\\\/.test(song.url)
                );
                ...
                alert(`成功导入 ${importedSongs.length} 首歌曲到收藏！`);
